...
ui <- fluidPage(
  # App title
  headerPanel("MDRClassifier: A tool for analyzing and classifying multidrug
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    if(input$sampleType == 'Single Isolate') {
      ...
      df <- classifyAllMDR(MDRdata(),input$antibioticColName)
      ...
      category <- classifyMDRfromRSI(RSIdata(), input$sampleRSI, input$totalCat)
      df <- classifyAllMDRfromRSI(RSIdata(),input$totalCat)
      p <- MDRClassifier::MDRPlot(df)
      ...
      if(input$plotType == 'individual'){
        p <- plotPCA(pcadata(), new_data=NULL,'individual')
      }
      else if(input$plotType == 'variables'){
        p <- plotPCA(pcadata(), new_data=NULL,'variables')
      }
